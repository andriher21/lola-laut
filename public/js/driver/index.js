...
var readCSV = '/readCSVdriver';
var deleteUrl = '/deletedriver';
var importURL = '/doImportdriver';
var restoreUrl = '/restoredriver';
var sync= '/syncdriver';
...
$('.employee-data').on('click', '.restore-token', function(){
    var id=$(this).attr('id');
        $.ajax({
            type: 'POST',
            url: base_url+restoreUrl,
            data: {id:id},
            success: function() {
...
function drawImportTable(data) {
    ...
    $.each(data, function(i, v) {
        ...
        // html += '<td>'+ ++count +' </td>'
        html += '<td id="nik">' + v.name + '</td>';
        html += '<td id="nama">' + v.nik + '</td>';
        html += '<td id="nama">' + v.address+ '</td>';
        html += '</tr>';
    });
    if (html !== '') {
