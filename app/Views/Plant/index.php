...
<div class="container-fluid employee-data">
    ...
    <div class="card-header py-3">
        <div class="float-left">
            <h6 class="m-0 font-weight-bold text-primary">Plant Data</h6>
            </div>
            <?php if ($_SESSION['role_id']=='4'){ ?>
                <div class="float-right">
                         <a class="btn btn-sm btn-danger btn-sync"><i class="fas fa-fw fa-sync" title="Sycnronitation Cloud X"></i>&nbsp;Sync Server X</a>
                 </div>        
         <?php
        }?>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-6">
                    <div class="input-group mb-3">
                        <input type="text" id="searchbox" class="form-control" placeholder="Search by Plant name">
                    ...
                    <tbody>
                        ...
                        foreach ($plant as $u) : ?>
                            <tr>
                                <td class="pr-4 pl-0">
                                    <div class="custom-control custom-checkbox table-checkbox">
                                        <input type="checkbox" class="chkboxes check check-data" name="checkbox" value="<?= $u['plant_id'] ?>" data-name="<?= $u['plant'] ?>">
                                    </div>
                                </td>
                                <td><?= $u['plant']; ?></td>
                                ...
                                <td><div class="btn-group">
                                    <?php if($_SESSION['role_id'] == '1' || $_SESSION['role_id'] == '4'){ ?>
                                        <button class="btn btn-sm btn-primary" data-toggle="modal" data-target="#modalEdit<?= $u['plant_id']; ?>" style="margin:auto; display:block;">
                                            <i class="fas fa-edit"></i>
                                        </button>
                                        <?php if($u['plant_status']=='1'){ ?>
                                        <button class="btn btn-sm btn-danger" data-toggle="modal" data-target=""style="margin:auto; display:block;">
                                            <i class="fas fa-sync"></i>
                                        </button>
                                        <?php }?>
                                    <?php }
                                    elseif($_SESSION['role_id'] == '2'){ ?>
                                        <button class="btn btn-sm btn-danger restore-token" id="<?= $u['plant_id']; ?>" style="margin:auto; display:block;" title="Restore Data">
    ...
    <div class="modal fade" id="modalCenter" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <form method="POST" action="<?= base_url('addplant') ?>" enctype="multipart/form-data">
                    <div class="modal-body">
                        <div class="form-group">
                            <div class="row">
                                <div class="col-md-12">
                                    <label for="message-text" class="col-form-label">Plant<small class="text-danger"><u>*</u></small></label>
                                    <input class="form-control" type="text" name="plant" required>
                                </div>
    ...
    <!-- Modal Edit Employee -->
    <?php foreach ($plant as $u) : ?>
        <div class="modal fade" id="modalEdit<?= $u['plant_id']; ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <!-- <h5 class="modal-title" id="exampleModalLongTitle">Delete Schedule</h5> -->

                        <h6 class="modal-title font-weight-bold text-primary"> Manage Plant</h6>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <form method="POST" action="<?= base_url('editplant') ?>" enctype="multipart/form-data">
                        <input type="hidden" name="id" value="<?= $u['plant_id'] ?>">
                        <div class="modal-body">
                            <div class="form-group">
                                <div class="row mb-2">
                                    <div class="col">
                                        <label class="col-form-label font-weight-bold text-dark">Plant</label>
                                        <input type="text" value="<?= $u['plant'] ?>" class="form-control" name="plant">
                                    </div>
                                </div>
    ...
    <div class="modal fade" id="importModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLongTitle">Import Plant</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
