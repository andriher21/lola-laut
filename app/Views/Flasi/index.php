...
<div class="container-fluid employee-data">
    ...
    <div class="card-header py-3">
        <div class="float-left">
            <h6 class="m-0 font-weight-bold text-primary">Flasi Data</h6>
            </div>
            <?php if ($_SESSION['role_id']=='4'){ ?>
                    <div class="float-right">
                            <a href="" class="btn btn-sm btn-danger btn-sync"><i class="fas fa-fw fa-sync" title="Sycnronitation Cloud X"></i>&nbsp;Sync Cloud X</a>
                    </div>     
         <?php
        }?>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-6">
                    <div class="input-group mb-3">
                        <input type="text" id="searchbox" class="form-control" placeholder="Search by Flasi name">
                    ...
                    <tbody>
                        ...
                        foreach ($flasi as $u) : ?>
                            <tr>
                                <td class="pr-4 pl-0">
                                    <div class="custom-control custom-checkbox table-checkbox">
                                        <input type="checkbox" class="chkboxes check check-data" name="checkbox" value="<?= $u['flasi_id'] ?>" data-name="<?= $u['flasi_plant'] ?>">
                                    </div>
                                </td>
                                <td><?= $u['flasi_plant']; ?></td>
                                <td><?= $u['flasi_material']; ?></td>
                                <td><?= $u['flasi_type_vcl']; ?></td>
                                <td><?= $u['flasi_ton']; ?></td>
                                <td><?= $u['flasi_cm']; ?></td>

                                <td>
                                    <?php if($_SESSION['role_id'] == '1' || $_SESSION['role_id'] == '4'){ ?>
                                        <button class="btn btn-sm btn-primary" data-toggle="modal" data-target="#modalEdit<?= $u['flasi_id']; ?>" style="margin:auto; display:block;">
                                            <i class="fas fa-edit"></i>
                                        </button>
                                    <?php }
                                    elseif($_SESSION['role_id'] == '2'){ ?>
                                        <button class="btn btn-sm btn-danger restore-token" id="<?= $u['flasi_id']; ?>" style="margin:auto; display:block;" title="Restore Data">
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLongTitle">Add Flasi</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form method="POST" action="<?= base_url('addflasi') ?>" enctype="multipart/form-data">
                    <div class="modal-body">
                        <div class="form-group">
                            <div class="row">
                                <div class="col-md-12">
                                    <label for="message-text" class="col-form-label">Plant<small class="text-danger"><u>*</u></small></label>
                                    <select name="plant" class="form-control">
                                            <option value="">Select Type Plant</option>
                                ...
                                <div class="col-md-12">
                                <label for="message-text" class="col-form-label">Type Vehicle<small class="text-danger"><u>*</u></small></label>
                                ...
                                <div class="col-md-12">
                                    <label for="message-text" class="col-form-label">Ton<small class="text-danger"><u>*</u></small></label>
                                    <input class="form-control" type="text" name="ton" required>
                                </div>
                                <div class="col-md-12">
                                    <label for="message-text" class="col-form-label">CM<small class="text-danger"><u>*</u></small></label>
                                    <input class="form-control" type="text" name="cm" required>
                                </div>
    ...
    <!-- Modal Edit Employee -->
    <?php foreach ($flasi as $u) : ?>
        <div class="modal fade" id="modalEdit<?= $u['flasi_id']; ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <!-- <h5 class="modal-title" id="exampleModalLongTitle">Delete Schedule</h5> -->

                        <h6 class="modal-title font-weight-bold text-primary"> Manage Flasi</h6>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <form method="POST" action="<?= base_url('editflasi') ?>" enctype="multipart/form-data">
                        <input type="hidden" name="id" value="<?= $u['flasi_id'] ?>">
                        <div class="modal-body">
                            <div class="form-group">
                                <div class="row mb-2">
                                    <div class="col">
                                        ...
                                        <select name="plant" class="form-control">
                                            <option value="">Select Type Plant</option>
                                            <?php foreach ($plant as $p) : ?>
                                                <option value="<?= $p['plant'] ?>" <?= $u['flasi_plant'] == $p['plant'] ? 'selected="true"' : ''; ?>><?= $p['plant']?> </option>
                                            <?php endforeach ?>
                                        </select>
                                    </div>
                                </div>
                                <div class="row mb-2">
                                    <div class="col">
                                        ...
                                        <input type="text" value="<?= $u['flasi_material'] ?>" class="form-control" name="material">
                                    </div>
                                </div>
                                <div class="row mb-2">
                                    <div class="col">
                                    <label for="message-text" class="col-form-label">Type Vehicle</label>
                                        <select name="type_vcl" class="form-control">
                                            <option value="">Select Type Vehicle</option>
                                            <?php foreach ($typevcl as $type) : ?>
                                                <option value="<?= $type['type_vcl'] ?>" <?= $u['flasi_type_vcl'] == $type['type_vcl'] ? 'selected="true"' : ''; ?>><?= $type['type_vcl']?> </option>
                                            <?php endforeach ?>
                                        </select>
                                    </div>
                                </div>
                                <div class="row mb-2">
                                    <div class="col">
                                        <label class="col-form-label font-weight-bold text-dark">Ton</label>
                                        <input type="text" value="<?= $u['flasi_ton'] ?>" class="form-control" name="ton">
                                    </div>
                                </div>
                                <div class="row mb-2">
                                    <div class="col">
                                        <label class="col-form-label font-weight-bold text-dark">CM</label>
                                        <input type="text" value="<?= $u['flasi_cm'] ?>" class="form-control" name="cm">
                                    </div>
                                </div>
    ...
    <div class="modal fade" id="importModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLongTitle">Import Flasi</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
