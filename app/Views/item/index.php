...
<div class="container-fluid employee-data">
                    ...
                    <tbody>
                        ...
                        foreach ($item as $u) : ?>
                            <tr>
                                <td class="pr-4 pl-0">
                                    <div class="custom-control custom-checkbox table-checkbox">
                                        <input type="checkbox" class="chkboxes check check-data" name="checkbox" value="<?= $u['item_id'] ?>" data-name="<?= $u['item_material'] ?>">
                                    </div>
                                </td>
                                <td><?= $u['item_plant']; ?></td>
                                <td><?= $u['item_material']; ?></td>
                                <td><?= $u['item_berat_jenis']; ?></td>
                                <td><?= $u['item_harga']; ?></td>
                                <td>
                                    <?php if($_SESSION['role_id'] == '1' || $_SESSION['role_id'] == '4'){ ?>
                                        <button class="btn btn-sm btn-primary" data-toggle="modal" data-target="#modalEdit<?= $u['item_id']; ?>" style="margin:auto; display:block;">
                                            <i class="fas fa-edit"></i>
                                        </button>
                                    <?php }
                                    elseif($_SESSION['role_id'] == '2'){ ?>
                                        <button class="btn btn-sm btn-danger restore-token" id="<?= $u['item_id']; ?>" style="margin:auto; display:block;" title="Restore Data">
    ...
    <div class="modal fade" id="modalCenter" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <form method="POST" action="<?= base_url('additem') ?>" enctype="multipart/form-data">
                    <div class="modal-body">
                        <div class="form-group">
                            <div class="row">
                                <div class="col-md-12">
                                <label for="message-text" class="col-form-label">Plant</label>
                                ...
                                <div class="col-md-12">
                                    <label for="message-text" class="col-form-label">Berat Jenis <small class="text-danger"><u>*</u></small></label>
                                    <input class="form-control" type="text" name="berat_jenis" required>
                                </div>
                                <div class="col-md-12">
                                    <label for="message-text" class="col-form-label">Harga <small class="text-danger"><u>*</u></small></label>
    ...
    <?php foreach ($item as $u) : ?>
        <div class="modal fade" id="modalEdit<?= $u['item_id']; ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <!-- <h5 class="modal-title" id="exampleModalLongTitle">Delete Schedule</h5> -->

                        <h6 class="modal-title font-weight-bold text-primary">Manage Item</h6>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <form method="POST" action="<?= base_url('edititem') ?>" enctype="multipart/form-data">
                        <input type="hidden" name="id" value="<?= $u['item_id'] ?>">
                        <div class="modal-body">
                            <div class="form-group">
                                <div class="row mb-2">
                                    ...
                                    <select name="plant" class="form-control">
                                            <option value="">Select Plant</option>
                                            <?php foreach ($plant as $p) : ?>
                                                <option value="<?= $p['plant'] ?>" <?= $u['item_plant'] == $p['plant'] ? 'selected="true"' : ''; ?>><?= $p['plant']?> </option>
                                            <?php endforeach ?>
                                        </select>
                                    </div>
                                </div>
                                <div class="row mb-2 ">
                                    <div class="col">
                                        ...
                                        <input type="text" value="<?= $u['item_material'] ?>" class="form-control" name="material">
                                    </div>
                                </div>
                                <div class="row mb-2 ">
                                    <div class="col">
                                        <label class="col-form-label font-weight-bold text-dark">Berat Jenis</label>
                                        <input type="text" value="<?= $u['item_berat_jenis'] ?>" class="form-control" name="berat_jenis">
                                    </div>
                                </div>
                                <div class="row mb-2 ">
                                    <div class="col">
                                        <label class="col-form-label font-weight-bold text-dark">Harga</label>
                                        <input type="text" value="<?= $u['item_harga'] ?>" class="form-control" name="harga">
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <div class="modal fade" id="importModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLongTitle">Import Item</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
